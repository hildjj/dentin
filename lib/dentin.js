...
class Dentin {
  ...
  _indent(stops) {
    this._spaces(this.opts.spaces * stops)
  }
  ...
  _printElement(node, parentKind, indent) {
    ...
    kind.mixed = kind.mixed || (parentKind && parentKind.mixed)
    ...
    if (!parentKind || !parentKind.mixed) {
      ...
      // and there's no PI in front of us, we don't want a newline.
      if (!this.first) {
        this._newline()
      }
      this._indent(indent)
