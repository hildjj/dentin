(function() {
  ...
  Denter = (function() {
    ...
    Denter.prototype._print_element = function(node, out, parent_kind, indent) {
      var a, attrs, c, cap, decls, first, i, j, k, kind, l, len1, len2, len3, len4, new_indent, ntxt, o, ps, ref, ref1, ref2, rm, tot;
      ...
      if (!this.noversion || (parent_kind != null) || (node.prevSibling() != null)) {
        ps = node.prevSibling();
        if (!((parent_kind != null ? parent_kind.mixed : void 0) && (ps != null) && (ps.type() === 'text') && (ps.text().trim().length > 1) && ps.text().match(/[^a-z0-9;: \.\n\f\t]\s*$/i))) {
          out("\n");
        }
      }
      out(spaces(this.indent_spaces * indent));
      ...
      if (kind.text || kind.elements) {
        ...
        if (kind.mixed) {
          ...
          for (l = 0, len3 = ref.length; l < len3; l++) {
            c = ref[l];
            this._print(c, cap, kind, 0);
          }
          this.right_margin = rm;
          this._print_text(cap.result, out, kind, new_indent);
        } else {
      ...
      if ((parent_kind != null ? parent_kind.mixed : void 0) && (((ref2 = node.nextSibling()) != null ? ref2.type() : void 0) === "text")) {
        ntxt = node.nextSibling().text();
        if (!ntxt.match(/\xA0/) && ntxt.match(/^\s*\w/)) {
          return out(" ");
        }
      }
    };

    Denter.prototype._print_text = function(t, out, parent_kind, indent) {
      var len, ttl;
      ttl = t.trim().length;
      ...
      switch (node.type()) {
        ...
        case 'text':
          if (ref = parent_kind.name, indexOf.call(this.ignore, ref) >= 0) {
            ...
            return this._print_text(escape(node.text()), out, parent_kind, indent);
          }
          break;
        case 'cdata':
