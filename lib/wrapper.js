...
/** @typedef {import('./dentin.js').Dentin} Dentin */
...
const BREAKS = new Rules({string: true});
...
export class Wrapper {
  /**
   * Child Wrappers, one per child node
   * @type {Wrapper[]}
   */
  children = [];
  ...
  /**
   * @type {xmljs.Node|xmljs.Document}
   */
  node;
   ...
   * @param {xmljs.Node|xmljs.Document|null} [node] Wrap the given node.
   * @param {Wrapper|null} [parentWrapper] The parent of this node, null for
   *   the top-level.
   */
   ...
   *
   * @param {number} [_indent=0] How many tab stops to indent.
   * @param {State} [state=null] The current print state.  New one created
   *   if null.
   * @returns {State} Final state.
   */
  print(_indent = 0, state = new State(this.dentin)) {
    return state;
  }
   ...
   *
   * @param {Dentin} dentin Configuration.
   * @param {xmljs.Node | xmljs.Document} [node=null] Wrap the given node.
...
class AttributeWrapper extends Wrapper {
  ...
  /**
   * @param {Dentin} dentin
   * @param {xmljs.Attribute} node
   * @param {Wrapper} parentWrapper
   */
  ...
  escapedValue() {
    ...
    const at = /** @type {xmljs.Attribute} */ (this.node);
    return at.value().replace(/[&<>"'\xA0]/g, m => ESCAPES[m]);
  }
...
class CdataWrapper extends Wrapper {
  ...
  print(indent = 0, state = undefined) {
    state = super.print(indent, state);
    const cd = /** @type {xmljs.Text} */ (this.node);
    const text = cd.text();
...
class TextWrapper extends Wrapper {
  ...
  print(indent = 0, state = undefined) {
    ...
    const txt = /** @type {xmljs.Text} */ (this.node);
    let t = txt.text();
    if (!this.comment) {
      t = escape(t);
    }
    ...
    if (parentMixed && !this.comment) {
      const n = /** @type {xmljs.Node} */(this.node);
    ...
    const ttl = state.width(t);
    let left = state.right;
    let right = (this.parentWrapper == null) ?
      0 :
      state.width(this.parentWrapper.name) + 3; // </name>
    ...
    for (let {string: chunk} of BREAKS.breaks(t)) {
      if (chunk.match(/^[^.]+\. $/)) {
      ...
      if (!first) {
        const trimChunk = chunk.trimEnd();
        if (lastLine + state.width(trimChunk) > this.dentin.opts.margin) {
...
class DocumentWrapper extends Wrapper {
  ...
  print(indent = 0, state = undefined) {
    ...
    if (!this.dentin.opts.noVersion) {
      if (!this.dentin.opts.html) {
        const doc = /** @type {xmljs.Document} */ (this.node);
        state.color`{PUNCTUATION <?}{ELEMENT xml} {ATTRIBUTE version}{PUNCTUATION =}{PUNCTUATION ${this.quote}}{ATTRIBUTE_VALUE ${doc.version()}}{PUNCTUATION ${this.quote}}?>`;
        state.newline();
      }
    }
...
class DeclWrapper extends Wrapper {
  ...
  print(indent = 0, state = undefined) {
    ...
    const startLen = state.width(decl);
...
class DtdWrapper extends Wrapper {
  ...
  print(indent = 0, state = undefined) {
    ...
    state.indent(indent);
    const n = /** @type {xmljs.Node} */(this.node);
    const {name, externalId, systemId} = n.doc().getDtd();
  ...
  print(indent = 0, state = undefined) {
    ...
    if (!this.nonempty && !this.elements) {
      ...
      if (this.mixed) {
        ...
        // </> -> 3
        if (!newlines &&
            (right + state.width(this.name) + 3 < this.dentin.opts.margin)) {
          // They fit!
...
class PiWrapper extends Wrapper {
  ...
  print(indent = 0, state = undefined) {
    ...
    if (parentMixed) {
      ...
      let first = true;
      while (n) {
        n = /** @type {xmljs.Node} */(n).prevSibling();
        if (n == null) {
          break;
        }
        if (n.type() === 'text') {
          const txt = /** @type {xmljs.Text} */ (n);
          if (!txt.text().match(/^\s*$/)) {
            first = false;
            break;
          }
    ...
    state.color`{PUNCTUATION <?}{ELEMENT ${this.name}}`;
    const pi = /** @type {xmljs.ProcessingInstruction} */ (this.node);
    let txt = pi.text();
    const tlen = state.width(txt);
    if (tlen > 0) {
      txt = txt.replace(
        /(?<q>['"])(?<txt>[^'"]+)\1/g,
...
class RefWrapper extends Wrapper {
  ...
  print(indent = 0, state = undefined) {
    state = super.print(indent, state);
    // @ts-expect-error No type for entity_ref.
